...
@section('home')
<div class="text-center fs-3">Edit Vaccine</div>
<form action="{{route('syncVaccine', $vaccine->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <label for="" class="form-text">Vaccine Name</label>
    <input type="text" name="nama_vaksin" id="" class="form-control mb-3" value="{{$vaccine->name}}">
    <label for="" class="form-text">Price</label>
    <div class="input-group mb-3">
        <span class="input-group-text" id="basic-addon1">Rp</span>
        <input type="text" name="harga" class="form-control" aria-label="Username" aria-describedby="basic-addon1" value="{{$vaccine->price}}">
    </div>
    <label for="" class="form-text">Description</label>
    <textarea name="deskripsi" id="" cols="30" rows="4" class="form-control mb-3">{{$vaccine->description}}</textarea>
